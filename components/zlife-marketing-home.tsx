    ...
    <main className="zlife-landing">
      ...
      <section id="home" className="zlife-hero">
        <div className="zlife-hero-copy">
          ...
          <div className="zlife-hero-actions">
            ...
            <a className="zlife-secondary" href="#modules">Explore Modules</a>
          </div>
          <div className="zlife-benefits">
            <span>⌁ Less overwhelm</span>
            <span>◎ AI that works for you</span>
            <span>▥ One connected platform</span>
            <span>◇ Private and human-centered</span>
          </div>
        </div>
        <div className="zlife-hero-art" aria-label="Z-Life vision">
          <div className="zlife-stars" />
          <div className="zlife-mountain mountain-back" />
          <div className="zlife-mountain mountain-front" />
          <div className="zlife-lake" />
      ...
      <section id="team" className="zlife-section">
        <div className="zlife-section-heading">
          <div><p className="zlife-kicker">AI TEAM</p><h2>Meet the AI Team</h2><p>Specialized agents with clear roles, working together inside Z-Life.</p></div>
      ...
      <section id="modules" className="zlife-section zlife-modules-section">
        <div className="zlife-section-heading">
          <div><p className="zlife-kicker">Z-LIFE MODULES</p><h2>One platform. Specialized modules.</h2><p>Every module starts with the Z-Life identity and connects back to the same assistant, data, and account.</p></div>
        </div>
          ...
          {modules.map((module) => {
            const content = (
              <>
                ...
                <div className="zlife-module-main"><span className="zlife-module-icon">{module.icon}</span><div><h3>{module.name.replace("Z-Life ", "")}</h3><p>{module.description}</p></div></div>
                {"nested" in module && module.nested ? <div className="zlife-nested-module"><span>▲</span><div><strong>Tree Service</strong><small>First active business vertical</small></div><b>ACTIVE</b></div> : null}
                <span className={`zlife-status ${module.status === "active" ? "is-active" : ""}`}>{module.status === "active" ? "Active Module" : "In Development"}</span>
              </>
            );
            const cls = `zlife-module-card ${module.status === "active" ? "is-active" : ""} ${"nested" in module ? "has-nested" : ""}`;
            return module.href ? <Link className={cls} href={module.href} key={module.name}>{content}</Link> : <article className={cls} key={module.name}>{content}</article>;
          })}
        </div>
      </section>
      ...
      <section id="vision" className="zlife-section zlife-vision-section">
        ...
        <div className="zlife-flow-grid">
          <article><span>01</span><h3>The AI Team</h3><p>Specialists handle product, architecture, design, engineering, operations, security, growth, and industry knowledge.</p></article>
          <article><span>02</span><h3>AI Assistant</h3><p>Your central guide surfaces what matters and connects you to the right module and next action.</p></article>
          <article><span>03</span><h3>Z-Life Modules</h3><p>Business, builders, home, money, family, services, and future modules share one connected platform.</p></article>
          <article><span>04</span><h3>A Better Tomorrow</h3><p>Less fragmentation, clearer work, simpler tools, and practical AI built around real people.</p></article>
        </div>
      </section>

      <section className="zlife-founder-section" aria-labelledby="founder-heading">
        <div className="zlife-founder-copy">
          <p className="zlife-kicker">FOUNDER SPOTLIGHT</p>
          <h2 id="founder-heading">Built by a real person solving real problems.</h2>
          <p>Brian Lehmann is the founder and builder behind Ziepher Tech and Z-Life. The platform is being built step by step around practical workflows, low-cost automation, and tools that reduce unnecessary complexity.</p>
          <div className="zlife-founder-points"><span>Vision + product direction</span><span>Websites, apps + business systems</span><span>Tree Service first launch focus</span><span>Practical, cost-aware automation</span></div>
        </div>
        <div className="zlife-founder-image-shell">
          <img src={founderCardImage} alt="Brian Lehmann, founder and builder of Ziepher Tech and Z-Life" />
          <div className="zlife-founder-tint" />
        </div>
      </section>
      ...
      <footer className="zlife-footer">
        ...
        <div className="zlife-footer-links"><a href="#home">Home</a><a href="#team">AI Team</a><a href="#modules">Modules</a><a href="#vision">Our Vision</a></div>
