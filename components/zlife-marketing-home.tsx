  ...
  return (
    <ZLifePublicShell>
      ...
      <section className="zlife-section">
        <div className="zlife-section-heading"><div><p className="zlife-kicker">WHAT Z-LIFE IS</p><h2>One platform with focused places to go.</h2><p>The homepage stays simple. Each major area now has its own page so visitors can understand Z-Life without scrolling through the entire company story at once.</p></div></div>
      ...
      <section className="zlife-section zlife-modules-section">
        <div className="zlife-section-heading"><div><p className="zlife-kicker">ACTIVE NOW</p><h2>Start with what already works.</h2><p>Tree Service is the first active business vertical, and the AI Assistant is an active Z-Life module.</p></div><Link href="/modules">View all modules →</Link></div>
        <div className="zlife-module-grid">{activeModules.map((item) => <Link className="zlife-module-card is-active" href={`/modules/${item.slug}`} key={item.slug}><div className="zlife-module-brand"><span className="module-z">Z</span><span>Z-LIFE</span></div><div className="zlife-module-main"><span className="zlife-module-icon" aria-hidden="true">{item.icon}</span><div><h3>{item.shortName}</h3><p>{item.description}</p></div></div>{item.nestedLabel ? <div className="zlife-nested-module"><span aria-hidden="true">▲</span><div><strong>Tree Service</strong><small>First active business vertical</small></div><b>ACTIVE</b></div> : null}<span className="zlife-status is-active">Active Module</span></Link>)}</div>
      </section>

      <section className="zlife-founder-section">
        <div className="zlife-founder-copy"><p className="zlife-kicker">BUILT FROM REAL-WORLD PROBLEMS</p><h2>From tattooing, to tree service, to building a simpler digital system.</h2><p>Z-Life grew from firsthand experience with creative work, small-business operations, AI, and the frustration of managing too many disconnected tools.</p><div className="zlife-hero-actions"><Link className="zlife-secondary" href="/about">Read the founder story</Link><Link className="zlife-secondary" href="/vision">See where Z-Life is going</Link></div></div>
      </section>
    </ZLifePublicShell>
  );
}
